-- =====================================================================
-- Migration 0033 — Stage-relevant funding notifications.
--
-- Funding listings use directory categories (undergraduate, honours,
-- masters, and so on). Notification matching must use those same categories;
-- an empty legacy relevant_stages array must never mean "notify everyone".
-- =====================================================================
...
begin
  ...
  if funding_alerts_enabled and target_stage is not null then
    insert into public.notifications (user_id, type, title, body, link, dedupe_key)
    select
      target_user,
      'funding',
    ...
    where f.is_published = true
      ...
      and case
        when target_stage in ('high_school', 'undergraduate')
          then coalesce(f.categories, '{}'::text[]) && array['undergraduate']::text[]
        when target_stage in ('honours_applicant', 'honours')
          then coalesce(f.categories, '{}'::text[]) && array['honours']::text[]
        when target_stage in ('masters_applicant', 'masters_student')
          then coalesce(f.categories, '{}'::text[]) && array['masters']::text[]
        when target_stage in ('intern', 'community_service', 'professional')
          then coalesce(f.categories, '{}'::text[]) &&
            array['doctoral', 'postdoctoral', 'research_grant', 'conference_travel']::text[]
        else false
      end
      and not exists (
        select 1 from public.notifications n
...
-- Remove only unread, automatically generated alerts that do not fit the
-- member's current stage. Saved-funding deadline reminders are preserved.
delete from public.notifications n
using public.funding_opportunities f, public.profiles p
where n.user_id = p.id
  and n.type = 'funding'
  and n.read = false
  and n.dedupe_key = 'new-funding:' || f.id::text
  and not case
    when p.career_stage::text in ('high_school', 'undergraduate')
      then coalesce(f.categories, '{}'::text[]) && array['undergraduate']::text[]
    when p.career_stage::text in ('honours_applicant', 'honours')
      then coalesce(f.categories, '{}'::text[]) && array['honours']::text[]
    when p.career_stage::text in ('masters_applicant', 'masters_student')
      then coalesce(f.categories, '{}'::text[]) && array['masters']::text[]
    when p.career_stage::text in ('intern', 'community_service', 'professional')
      then coalesce(f.categories, '{}'::text[]) &&
        array['doctoral', 'postdoctoral', 'research_grant', 'conference_travel']::text[]
    else false
  end;
